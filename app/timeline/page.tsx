  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/air/history?period=${selectedPeriod}&userId=${user.id}&lat=${selectedLocation.lat}&lon=${selectedLocation.lon}`
      );
      if (!response.ok) {
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center gap-4">
                                  <div
                                    className={`w-12 h-12 rounded-full flex items-center justify-center ${aqiInfo.bgColor}`}
                                  >
                                    <span
                                      className={`text-lg font-bold ${aqiInfo.textColor}`}
                                    >
                                      {reading.aqi}
                                    </span>
                                  </div>
                                  <div>
                                    <div className="flex items-center gap-2 mb-1">
                                      ...
                                      <Badge
                                        className={`${aqiInfo.bgColor} ${aqiInfo.textColor} text-xs`}
                                      >
                                        {reading.category}
                                      </Badge>
                                    </div>
