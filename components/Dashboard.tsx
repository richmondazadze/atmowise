...
export default function Dashboard() {
  ...
  const airQualityLat = selectedLocation?.lat || (currentLat && !isNaN(currentLat) ? currentLat : fallbackLat);
  const airQualityLon = selectedLocation?.lon || (currentLon && !isNaN(currentLon) ? currentLon : fallbackLon);
  
  const { data: airQualityData, isLoading: airQualityLoading, refetch: refetchAirQuality, error: airQualityError } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        return {
          ...
          category: 'Moderate',
          dominantPollutant: 'PM2.5',
          rawPayload: { demo: true, error: error instanceof Error ? error.message : 'Unknown error' },
          demo: true,
          error: true
        };
      }
  ...
  const handleLocationSelect = (location: { lat: number; lon: number; label: string }, airQuality: any) => {
  ...
  const getAQIInfo = (aqi: number | null) => {
    if (!aqi) return { label: 'Unknown', color: 'bg-gray-500', textColor: 'text-gray-500' };
    ...
    if (aqi <= 150) return { label: 'Unhealthy for Sensitive Groups', color: 'bg-orange-500', textColor: 'text-orange-600' };
    if (aqi <= 200) return { label: 'Unhealthy', color: 'bg-red-500', textColor: 'text-red-600' };
    if (aqi <= 300) return { label: 'Very Unhealthy', color: 'bg-purple-500', textColor: 'text-purple-600' };
    return { label: 'Hazardous', color: 'bg-red-800', textColor: 'text-red-800' };
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                <h1 className="text-xl font-bold text-[#0A1C40] truncate tracking-tight">AtmoWise</h1>
                <p className="text-xs text-[#64748B] truncate font-medium">Air Quality Dashboard</p>
              </div>
            </div>
            <div className="flex items-center space-x-2 flex-shrink-0">
              ...
              >
                ...
                <span className="hidden xs:inline truncate max-w-24 font-medium">
                  {selectedLocation ? selectedLocation.label.split(',')[0] : 'Location'}
                </span>
              </Button>
              <Button
                variant="outline"
        ...
        <div className="px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="body-large text-[#64748B]">Monitor air quality and your health insights</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-4 lg:space-y-8">
            ...
            <div className="bg-white rounded-2xl p-5 lg:p-8 animate-fade-in shadow-sm border border-gray-100/50">
              <h3 className="text-lg lg:text-xl font-bold text-[#0A1C40] mb-5 lg:mb-6 tracking-tight">Log Symptoms</h3>
              <SymptomForm
                userId={supabaseUser?.id}
                profile={profile}
                airData={airQualityData}
          ...
          <div className="space-y-4 lg:space-y-6">
            ...
            <div className="bg-white rounded-2xl p-5 lg:p-6 hover:shadow-lg transition-all duration-300 shadow-sm border border-gray-100/50">
              <h3 className="text-lg lg:text-xl font-bold text-[#0A1C40] mb-5 lg:mb-6 tracking-tight">Quick Stats</h3>
              <div className="space-y-3 lg:space-y-4">
                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-gray-100/50 rounded-xl border border-gray-100">
                  <span className="text-sm lg:text-base text-[#64748B] font-semibold">Air Quality</span>
                  <span className={`px-3 py-1.5 rounded-full text-xs lg:text-sm font-bold ${aqiInfo.color} text-white shadow-sm`}>
                    {aqiInfo.label}
                  </span>
                </div>
                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-gray-100/50 rounded-xl border border-gray-100">
                  <span className="text-sm lg:text-base text-[#64748B] font-semibold">Symptoms Logged</span>
