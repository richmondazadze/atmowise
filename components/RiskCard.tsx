...
import { Wind, Activity, AlertTriangle, CheckCircle, Clock, RefreshCw } from "lucide-react";
...
export function RiskCard({ airData, profile, isLoading, lastUpdated }: RiskCardProps) {
  // Calculate risk status based on AQI and user sensitivity
  const getRiskStatus = (aqi: number | null, sensitivity: any) => {
    if (!aqi) return { status: 'Unknown', color: 'bg-gray-500', textColor: 'text-gray-600', icon: AlertTriangle };
    
    // Adjust thresholds based on sensitivity
    const isHighRisk = sensitivity?.asthma || sensitivity?.pregnant || sensitivity?.cardiopulmonary;
    const ageAdjustment = sensitivity?.ageGroup === 'child' || sensitivity?.ageGroup === 'senior' ? -20 : 0;
    ...
    if (aqi <= adjustedThresholds.good + ageAdjustment) {
      return { 
        ...
        icon: CheckCircle,
        advice: isHighRisk ? 'Great air quality! Perfect time for outdoor activities.' : 'Excellent air quality for everyone.'
      };
    }
    if (aqi <= adjustedThresholds.moderate + ageAdjustment) {
      return { 
        ...
        icon: AlertTriangle,
        advice: isHighRisk ? 'Consider limiting outdoor activities. Use air purifier indoors.' : 'Moderate air quality. Sensitive individuals should limit outdoor exertion.'
      };
    }
    if (aqi <= adjustedThresholds.unhealthy + ageAdjustment) {
      return { 
        ...
        icon: AlertTriangle,
        advice: isHighRisk ? 'Stay indoors. Use air purifier and avoid outdoor activities.' : 'Unhealthy air quality. Limit outdoor activities and consider wearing a mask.'
      };
    }
    return { 
      status: 'Avoid', 
      ...
      icon: AlertTriangle,
      advice: 'Very unhealthy air quality. Stay indoors and avoid all outdoor activities.'
    };
  };
    ...
    <div className="bg-white rounded-2xl p-5 lg:p-6 hover:shadow-lg transition-all duration-300 shadow-sm border border-gray-100/50">
      <div className="pb-4 lg:pb-4">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 lg:w-12 lg:h-12 ${riskInfo.color} rounded-2xl flex items-center justify-center shadow-lg ring-2 ring-white/20`}>
      ...
      <div className="space-y-4">
        ...
        <div className="text-center p-5 lg:p-6 bg-gradient-to-br from-[#F8FAFC] via-[#F1F5F9] to-[#E2E8F0] rounded-2xl border border-gray-100/50">
          <div className="flex items-center justify-center gap-3 mb-3">
            <Badge className={`${riskInfo.color} text-white text-sm lg:text-base px-4 py-2 rounded-full font-bold shadow-sm`}>
              {riskInfo.status}
            </Badge>
            {airData?.aqi && (
        ...
        {dominantPollutant && (
          <div className="flex items-center justify-center">
            <Badge variant="outline" className="text-xs border-gray-200 text-[#64748B] px-3 py-1.5 rounded-full font-medium">
        ...
        <div className="flex items-center justify-between text-xs text-[#64748B] bg-gray-50/50 rounded-xl p-3">
          ...
          {airData?.source && (
            <Badge variant="outline" className="text-xs px-2 py-1 rounded-full border-gray-200 font-medium">
              {airData.source === 'openweather' ? 'OpenWeather' : 
               airData.source === 'airnow' ? 'AirNow' : 
               'Demo Data'}
            </Badge>
          )}
              </div>
